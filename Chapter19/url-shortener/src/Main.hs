...
checkURI :: R.Connection
         -> BC.ByteString
         -> IO (Either R.Reply Bool)
checkURI conn shortURI = R.runRedis conn $ R.exists shortURI
  ...
  get "/" $ do
    ...
    case parsedUri of
      Just _ -> do
        ...
        rreply <- liftIO (checkURI rConn shorty)
        case rreply of
          Left reply -> text (TL.pack (show reply))
          Right True -> text (TL.pack ("Sorry, caught collision, try again"))
          Right False -> do
